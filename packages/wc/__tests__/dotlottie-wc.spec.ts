...
const workerUrl = new URL('../../web/dist/dotlottie-worker.js', import.meta.url).href;
...
describe.each([
  { elementName: 'dotlottie-wc' as const, isWorker: false },
  { elementName: 'dotlottie-worker-wc' as const, isWorker: true },
])('$elementName', ({ elementName, isWorker }) => {
  const getWorkerAttrs = (): Record<string, string> => (isWorker ? { workerUrl } : {});
  ...
  test('should instantiate dotLottie instance on connectedCallback', () => {
    const { element } = render(elementName, { ...getWorkerAttrs(), src });
  ...
  test('calls dotLottie.destroy on unmount', async () => {
    const { element, unmount } = render(elementName, { ...getWorkerAttrs(), src });
  ...
  test('should render with data', async () => {
    ...
    const { element } = render(elementName, { ...getWorkerAttrs(), data: animationData });

    expect(element).toBeDefined();
  });

  test('calls dotLottie.setSpeed when speed attribute changes', async () => {
    const { element, rerender } = render(elementName, { ...getWorkerAttrs(), src });
  ...
  test('calls dotLottie.load when src attribute changes', async () => {
    const { element, rerender } = render(elementName, { ...getWorkerAttrs() });
  ...
  test('calls dotLottie.setBackgroundColor when background-color attribute changes', async () => {
    ...
    rerender({ ...getWorkerAttrs(), backgroundcolor: '#ff00ff' });
  ...
  test('calls dotLottie.setRenderConfig when renderconfig attribute changes', async () => {
    ...
    rerender({ ...getWorkerAttrs(), renderconfig: JSON.stringify({ devicePixelRatio: 0.5 }) });
  ...
  test('calls dotLottie.loadAnimation when animationid attribute changes', async () => {
    ...
    rerender({ ...getWorkerAttrs(), animationid: animationId as string });
    ...
    rerender({ ...getWorkerAttrs(), animationid: '' });
  ...
  test('calls stateMachineLoad and stateMachineStart when statemachineid attribute changes', async () => {
    const { element, rerender } = render(elementName, { ...getWorkerAttrs(), src: smSrc });
    ...
    rerender({ ...getWorkerAttrs(), src: smSrc, statemachineid: 'testSM' });
    ...
    rerender({ ...getWorkerAttrs(), src: smSrc });
  ...
  test('calls stateMachineSetConfig when statemachineconfig attribute changes', async () => {
    ...
    rerender({ ...getWorkerAttrs(), src: smSrc, statemachineconfig: JSON.stringify(config) });
